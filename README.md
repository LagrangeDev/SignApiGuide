...
```
#signer bind <要绑定的 QQ 号 0> <要绑定的 QQ 号 1> <要绑定的 QQ 号 2> ...
```

> [!tip]
> 由于 Bot 存在 1 分钟/次的 rate limit，请尽量一次性绑定多个 QQ 号。
...
```
#signer unbind <要解绑的 QQ 号 0> <要解绑的 QQ 号 1> <要解绑的 QQ 号 2> ...
```

> [!tip]
> 同样出于 rate limit 的考虑，请尽量一次性解绑多个 QQ 号。
...
如果你遗忘了自己的 token，或者不再需要使用 Lagrange V2 Sign API，可以通过以下命令注销你的账号：
...
> [!warning]
> 账号注销后，3 天内将不允许重新注册。请确认你确实不再需要使用该服务或无法找到 token 后，再执行此操作。
